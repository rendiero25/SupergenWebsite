...
const Products = () => {
  ...
  const productsData =
    language === "Indonesia" ? productsDataId : productsDataEn;
  ...
  // Find Mapping
  const mainCategory = productsData.find(
    (p) => p.name.toLowerCase().replace(/\s+/g, "-") === mainCategorySlug,
  );
  ...
  const currentSubCategory = useMemo<SubCategory | null>(() => {
    ...
    const found = mainCategory.type?.find(
      (t) => t.name.toLowerCase().replace(/\s+/g, "-") === subCategorySlug,
    );
  ...
  const brands = useMemo<Brand[]>(() => {
    ...
    const typeKey = keys.find(
      (k) =>
        k.endsWith("type-type") ||
        (Array.isArray(currentSubCategory[k]) && k !== "image-banner"),
    );
  ...
  const selectedBrand = useMemo<Brand | null>(() => {
    ...
    return (
      brands.find(
        (b) => b.name.toLowerCase().replace(/\s+/g, "-") === brandSlug,
      ) || null
    );
  }, [brandSlug, brands]);
    ...
    if (generatorSet && generatorSet.type) {
      ...
      if (openType) {
        ...
        const typeKey = keys.find(
          (k) =>
            k.endsWith("type-type") ||
            (Array.isArray(openType[k]) && k !== "image-banner"),
        );
        if (typeKey) {
  ...
  const renderBrandList = (subCatSlug: string) => {
    if (
      ...
      mainCategorySlug !== "generator-set" ||
      brands.length === 0
    ) {
      return null;
    }
      ...
      <div className="bg-gray-50 border-b border-gray-100">
        {brands.map((brand) => {
          ...
          return (
            ...
            >
              <span
                className={`w-1.5 h-1.5 rounded-full ${isSelected ? "bg-[#E60013]" : "bg-gray-300"}`}
              ></span>
              {brand.name}
            </Link>
          );
  ...
  return (
    <div className="bg-gray-50 min-h-screen font-outfit">
      ...
      <div className="w-full bg-white">
        <div className="container mx-auto bg-white px-4 md:px-8 lg:px-15 xl:px-15 2xl:px-20 3xl:px-0 py-7 mb-5 flex items-center text-sm text-black gap-1 flex-wrap">
          ...
          {mainCategory && (
            <>
              ...
              >
                {mainCategory.name === "Generator Set"
          ...
          {currentSubCategory && mainCategorySlug === "generator-set" && (
            <>
              ...
              <Link
                ...
                className={
                  !brandSlug ? "text-red-600 font-bold" : "hover:text-red-600"
                }
              >
                {currentSubCategory.name === "Open Type"
                  ? t("products.openType")
                  : currentSubCategory.name === "Silent Type"
            ...
            <div className="bg-white shadow-sm border border-gray-200">
              ...
              <div>
                ...
                >
                  ...
                  <IoIosArrowDown
                    className={`transition-transform duration-300 ${isGenSetOpen ? "rotate-180" : ""}`}
                  />
                </button>

                <div
                  className={`overflow-hidden transition-all duration-300 ${isGenSetOpen ? "max-h-[2000px]" : "max-h-0"}`}
                >
                  {/* Open Type */}
                  <div>
                    <Link
                      to="/products/generator-set/open-type"
                      className={`flex items-center gap-2 px-5 py-3 text-sm transition-colors border-b border-gray-100 ${
                        subCategorySlug === "open-type" &&
                        mainCategorySlug === "generator-set"
                  ...
                  <div>
                    <Link
                      to="/products/generator-set/silent-type"
                      className={`flex items-center gap-2 px-5 py-3 text-sm transition-colors border-b border-gray-100 ${
                        subCategorySlug === "silent-type" &&
                        mainCategorySlug === "generator-set"
                          ? "text-[#E60013] font-medium"
                          : "text-gray-600 hover:text-[#E60013]"
                      }`}
                  ...
                  <div>
                    <Link
                      to="/products/generator-set/container-type"
                      className={`flex items-center gap-2 px-5 py-3 text-sm transition-colors border-b border-gray-100 ${
                        subCategorySlug === "container-type" &&
                        mainCategorySlug === "generator-set"
                          ? "text-[#E60013] font-medium"
                          : "text-gray-600 hover:text-[#E60013]"
                      }`}
              ...
              <div className="border-t border-gray-200">
                <button
                  ...
                  className={`w-full flex items-center justify-between px-5 py-4 text-left transition-colors ${
                    isBrushlessOpen ||
                    mainCategorySlug === "brushless-alternator"
                      ? "bg-red/10 text-black"
                      : "bg-white text-gray-700 hover:text-[#E60013]"
                  }`}
                >
                  ...
                  <IoIosArrowDown
                    className={`transition-transform duration-300 ${isBrushlessOpen ? "rotate-180" : ""}`}
                  />
                </button>
        ...
        <div className="lg:col-span-3">
          {selectedBrand ? (
            ...
            <div className="bg-white">
                      ...
                      <div className="flex gap-2 overflow-x-auto">
                        {productImages.map((img, idx) => (
                          <button
                            ...
                            className={`shrink-0 w-20 h-20 border-2 p-1 transition-colors ${
              ...
              <div className="py-6 bg-gray-50">
                ...
                {(() => {
                  ...
                  const hzTypeKey = keys.find(
                    (k) =>
                      k.endsWith("-type") && Array.isArray(selectedBrand[k]),
                  );
                  const hzTypes = hzTypeKey
                    ? (selectedBrand[hzTypeKey] as Record<string, unknown>[])
                    : [];

                  if (hzTypes.length > 0) {
                    const currentHz = hzTypes[selectedHzTab] || hzTypes[0];
                    const descKeys = Object.keys(currentHz);
                    const desc1Key = descKeys.find((k) =>
                      k.includes("description1"),
                    );
                    const desc2Key = descKeys.find((k) =>
                      k.includes("description2"),
                    );
                    const imageKey = descKeys.find(
                      (k) =>
                        k.includes("image") &&
                        !k.includes("image1") &&
                        !k.includes("image2"),
                    );
                    const image1Key = descKeys.find((k) =>
                      k.includes("image1"),
                    );
                    const image2Key = descKeys.find((k) =>
                      k.includes("image2"),
                    );
                    ...
                    return (
                      <div>
                        ...
                        <div className="bg-white p-6">
                          ...
                          {desc1Key &&
                            typeof currentHz[desc1Key] === "string" && (
                          ...
                          <div className="flex gap-4 my-4">
                            {imageKey &&
                              typeof currentHz[imageKey] === "string" && (
                                <img
                            ...
                            {image1Key &&
                              typeof currentHz[image1Key] === "string" && (
                                <img
                            ...
                            {image2Key &&
                              typeof currentHz[image2Key] === "string" && (
                                <img
                          ...
                          {desc2Key &&
                            typeof currentHz[desc2Key] === "string" && (
            ...
            <div className="bg-white">
              ...
              {/* Brand Grid */}
              {brands.length > 0 && (
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3 px-4 sm:px-7 pb-7">
                  {brands.map((brand) => (
                    <Link
                      key={brand.id}
                    ...
                    >
                      <div className="h-28 sm:h-32 md:h-40 flex items-center justify-center mb-2 sm:mb-4 w-full">
                        <img
                          src={brand.logo}
                          alt={brand.name}
                      ...
                      <h3 className="text-xs sm:text-sm font-bold uppercase text-gray-800 text-center mt-5 md:mt-0">
                        {brand.name}
                      </h3>
                      <span className="text-xs sm:text-sm text-gray-400 px-3 sm:px-5 py-1 sm:py-2 my-1 sm:my-2 group-hover:text-[#E60013] group-hover:bg-red group-hover:text-white transition-colors">
                        {t("common.clickHere")}
                      </span>
                    </Link>
                  ))}
      ...
      {selectedBrand && relatedProducts.length > 0 && (
        <div
          ...
          style={{
            backgroundImage: "url('/images/products/productrelated-bg.jpg')",
          }}
        >
        ...
        <div className="w-full py-15 px-6 bg-gray-100">
          <div className="container mx-auto">
            ...
            <p className="text-sm text-black text-center mb-8">
              {t("inquiry.replyTime")}{" "}
              <span className="text-[#E60013]">{t("inquiry.hours")}</span>.
            </p>
            ...
            <form className="max-w-4xl mx-auto space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <input
                  ...
                  defaultValue={
                    selectedBrand?.name
                      ? `Product Name:${selectedBrand.name}`
                      : ""
                  }
